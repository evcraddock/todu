...
function CommentThread({ taskId }: { taskId: string }): ReactNode {
  const { data: notes, isLoading } = useNotes({ entityType: "task", entityId: taskId });
  ...
  const handleSubmit = () => {
    ...
    createNote.mutate(
      { content: newComment.trim(), entityType: "task", entityId: taskId, author: "user" },
