...
# Create varied tasks
todu --format json task create --title "Fix crash" --project "App" --priority high --label bug --due "2020-01-01"
todu --format json task create --title "Add search" --project "App" --priority medium --label feature --due "$(date +%Y-%m-%d)"
TASK3=$(todu --format json task create --title "Setup CI" --project "Infra" --priority low)
TASK3_ID=$(echo "$TASK3" | jq -r .id)
todu task start "$TASK3_ID"
todu --format json task create --title "Write docs" --project "App" --priority low --scheduled "$(date +%Y-%m-%d)"
TASK5=$(todu --format json task create --title "Old task" --project "App" --priority medium)
TASK5_ID=$(echo "$TASK5" | jq -r .id)
todu task done "$TASK5_ID"
```
...
```bash
todu task list --status active --no-color
```
...
```bash
todu task list --status active,inprogress --no-color
```
...
```bash
todu task list --priority high --no-color
```
...
```bash
todu task list --project "Infra" --no-color
```
...
```bash
todu task list --label bug --no-color
```
...
```bash
todu task list --overdue --no-color
```
...
```bash
todu task list --status active --priority medium --no-color
```
...
```bash
todu --format json task list --status active
```
