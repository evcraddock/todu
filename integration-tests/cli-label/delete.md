...
```bash
LABEL_ID=$(todu --format json label list | jq -r '.[] | select(.name == "docs") | .id')
todu label delete "$LABEL_ID"
```
...
TASK=$(todu --format json task create --title "Test task" --project "App" --label feature)
TASK_ID=$(echo "$TASK" | jq -r .id)

# Verify label is on the task
todu task show "$TASK_ID" --no-color | grep "Labels"
```

**Expected:** `Labels: feature`

```bash
todu label delete feature
...
```bash
todu label create --name temp --color "#3b82f6"
NODE_PATH=$NODE_PATH node $INTERACT click "text=Labels"
NODE_PATH=$NODE_PATH node $INTERACT wait ".data-table" --timeout 5000
NODE_PATH=$NODE_PATH node $INTERACT text --selector ".data-table"
```
...
```bash
todu label create --name important --color "#ef4444"
todu task create --title "Important task" --project "App" --label important
