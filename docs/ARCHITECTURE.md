...
- Daemon plugin module paths resolve from `TODU_DAEMON_PLUGIN_PATHS` first, then `daemon.plugins.paths` in config; config file paths resolve relative to config location.
...
- Environment override uses `TODU_DAEMON_ASSIGNED_WORKERS` (comma-separated worker types) and takes precedence over file assignment.
